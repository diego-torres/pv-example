...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "pv-example") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name pv-example apps/pv-example:1.0
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app pv-example:1.0
		call oc expose svc/pv-example
	) else (
